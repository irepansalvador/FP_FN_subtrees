...
my $Alleles_file = $output_dir.$backboneFILE."_ALLELES_ordered";
...
($div) = ($div =~ m/(\d+)/);
print "my div = $div\n";
my $number_taxa= 1024+1;
#if ($div >=10) {$number_taxa = 1000 +1} #Also will add root
#else {$number_taxa= (2**$div) +1}

my $n_samples=64; # determine the number of cells (max 1K, 10 random samples for div >=10)
...
for $sample (1..$n_samples)
	{
	my $c  = ($sample-1) * ($number_taxa-1);
	my $cc = $sample * ($number_taxa-1);
#	print "SAMPLE #$sample\n";
	##################################################################
	if ($weight == 1)
	{
	$PAUP_tree = $backboneFILE."_sample_".$sample."_Weighted_PAUP_ordered.tre";
	$nexus = $nexus_dir.$backboneFILE."_sample_".$sample."_Weighted_ordered.nexus";		
	}
	if ($weight == 0)
	{
	$PAUP_tree = $backboneFILE."_sample_".$sample."_Unweighted_PAUP_ordered.tre";
	$nexus = $nexus_dir.$backboneFILE."_sample_".$sample."_Unweighted_ordered.nexus";		
	}
...
print NEXUS "begin paup;
	...
	#}
#	##################################################################
#	#run paup on nexus file to produce test tree based on mutated loci
#	##################################################################
#	#print "\t 3) finding tree with PAUP\n";
#	system "../../../Notebooks/perl_scripts/paup4a153_ubuntu64 $nexus > /dev/null";
#	
	##################################################################
...
#	##################################################################
#	#open control tree and nexus (test) tree put into same file (need to edit some stuff out of nexus tre)
#	##################################################################
#	my $nexus_tree = $ARGV[0];
#	for (1..4) {chop $nexus_tree};
#	
#	$nexus_tree = $nexus_tree."_subset.nw";
#	
#	open (FILE1, "$PAUP_tree");
#	open (NEWNEXUSTRE, "> $nexus_tree");
#	
#	while (<FILE1>) {
#		chomp;
#		if ($_ =~ m/PAUP_1/) {
#			$_ =~ s/tree\s'PAUP\_1'\s\=\s\[\&U\]\s//;
#			print NEWNEXUSTRE $_;
#			}
#		else {next;}
#		}
...
#	############ Compare trees
#	
##	print "\t 4) comparing to reference tree \n\n";
#	system "../../../Notebooks/perl_scripts/CompareTree_MOTree.pl -simplify -tree $ref -versus $nexus_tree > /dev/null  2>> $output";
#		

#
### with a adhoc R script, calculate the mean and SD from the samples and generate a table
#
#system "../../../Notebooks/R_scripts/samples_mean_sd.R $output $rep";
#system "../../../Notebooks/R_scripts/samples_mean_sd.R $Alleles_file $rep";
