...
#	print ("tree $x = $CELLS[$x] cells, lca in REF = $ALL_CELLS[$x]\n");
	#$FN = $FN + ( ($ALL_CELLS[$x] - $CELLS[$x] )/ $ALL_CELLS[$x]);
	}

$FN_global = sprintf("%.4f", $FN / ($incorrect_T + $correct_T) );
$RF = sprintf("%.4f", $correct_T / ($incorrect_T + $correct_T) );
#$FN = sprintf("%.4f", $FN / $incorrect_T);
#$FN_cells = sprintf("%.4f", $FN_cells / $incorrect_T);
...
#print ("FN\t\tAcc\t\tOK_Tree\tIncorrect_Trees\n" );

print ("$FN_global\t$RF\t$correct_T\t$incorrect_T\n" );	
