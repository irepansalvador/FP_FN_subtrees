...
$out1 = "../4cell_trees_SHUFFLED/$output";
...
if ($cells <= 1000)
	...
	for $i (1..$Ntrees)
		...
		$out2 = $output."_".$i;
		#system 	"echo Rscript ../../R_scripts/subtree_nj_shuffle.R $seqs[$ii] $seqs[$ii+1] $seqs[$ii+2] $seqs[$ii+3] $out2";			
		system 	"Rscript ../../R_scripts/subtree_nj_shuffle.R $seqs[$ii] $seqs[$ii+1] $seqs[$ii+2] $seqs[$ii+3] $out2";	
