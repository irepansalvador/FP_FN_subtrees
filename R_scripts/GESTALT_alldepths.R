...
for (i in trees)  ## for each output file (which contains 50 results)
{
  for (ii in samples)  ## for each output file (which contains 50 results)
  {
    ...
    my.result = read.table(file = paste("../GESTALT/ALL_results/FP_t",my_tree,sep = ""),
...
svg(filename = "GESTALT_FP_alldepths_1Ksims_10samples.svg",width = 4,height = 4)

layout(1:2, heights=c(1, 5))
# Legend panel
par(mar=rep(0,4))
plot(0, 0, type="n", ann=FALSE, axes=FALSE)
legend("center",c("Dropouts (GESTALT)", "Random"), horiz=TRUE, title = "False Positives",
